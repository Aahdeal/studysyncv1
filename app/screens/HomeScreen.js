...
const HomeScreen = ({ navigation, user }) => {
  ...
  useEffect(() => {
    ...
    if (taskData.length > 0 && completeTaskData.length > 0 && eventData.length > 0) {
        calcTaskCompletion();
        calcHoursStudied();
    }
  ...
  const calcHoursStudied = () => {
    ...
    let duration = []; // Initialize total duration
        ...
        for (const { event } of eventsSinceMonthStart) {
            ...
            const eventDuration = (endDate - new Date(event.startDate)) / (1000 * 60 * 60); // Convert milliseconds to hours
  ...
  const calcTaskCompletion = () => {
    const completeTasks = Array.isArray(completeTaskData) ? completeTaskData.length : 0;
    const totalTasks = Array.isArray(completeTaskData) && Array.isArray(taskData)
